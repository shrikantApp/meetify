...
export function ConversationItem({ conversation, active, onClick }: Props) {
  ...
  const directAvatarUrl =
    conversation.type === "direct"
      ? (otherMember?.user as any)?.avatarUrl ||
        (otherMember?.user as any)?.avatar ||
        null
      : null;
  ...
  return (
    <div
      ...
      className={`
        ...
        ${
          active
            ? "bg-white/10 text-[var(--slack-sidebar-text)] shadow-sm"
            : "hover:bg-white/5 text-sidebar-text hover:text-[var(--slack-sidebar-text)]"
        }
    ...
    >
      <div className="relative flex-shrink-0">
        {conversation.type === "group" ? (
          <div
            ...
            ${active ? "bg-[var(--accent-primary)] text-[var(--slack-sidebar-text)]" : "bg-white/5 text-sidebar-text 40 group-hover:bg-white/10"}
          `}
          >
          ...
          <div className="relative">
            {directAvatarUrl ? (
              ...
              <div
                className={`
                w-7 h-7 rounded-lg premium-gradient flex items-center justify-center text-[var(--slack-sidebar-text)] text-[10px] font-bold shadow-sm
                ${active ? "ring-2 ring-[var(--accent-primary)] ring-offset-2 ring-offset-[var(--bg-sidebar)]" : ""}
              `}
              >
      ...
      <div className="flex-1 min-w-0 flex items-center justify-between">
        <span
          className={`text-[13px] truncate ${active ? "font-bold" : "font-semibold"}`}
        >
          {name}
        </span>
        ...
        {hasUnread && !active && (
          <div className="min-w-[16px] h-[16px] px-1 rounded-full bg-[var(--accent-primary)] flex items-center justify-center">
            <span className="text-[9px] font-bold text-[var(--slack-sidebar-text)]">
