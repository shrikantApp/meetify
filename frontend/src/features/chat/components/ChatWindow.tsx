...
export function ChatWindow({ conversationId, onOpenThread }: Props) {
  ...
  const loading = useAppSelector(
    (state) => state.chat.isLoadingMessages[conversationId],
  );
      ...
      >
        <div className="flex flex-col py-4">
          ...
          <div className="px-6 mb-8 mt-4 welcome-section-compact">
            ...
            <h1 className="text-xl font-bold text-[var(--text-primary)] mb-1.5">
              Welcome to this conversation!
            </h1>
            <p className="text-[13px] text-[var(--text-secondary)] max-w-sm leading-relaxed">
              This is the very beginning of your collaboration. Share files,
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        <ChevronDown
          className={`w-3.5 h-3.5 text-[var(--text-muted)] group-hover:text-[var(--text-primary)] transition-transform ${isOpen ? "rotate-180" : ""}`}
        />
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="px-3 py-2 text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest border-b border-[var(--border-subtle)] mb-1">
              Jump to...
            </div>

            {options.yesterday && (
              <button className="w-full text-left px-3 py-2 hover:bg-white/5 rounded-lg text-[12px] text-[var(--text-primary)] transition-colors">
                Yesterday
              </button>
            )}
            {options.lastWeek && (
            ...
            <button className="w-full text-left px-3 py-2 hover:bg-white/5 rounded-lg text-[12px] text-sky-400 font-medium transition-colors">
