      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-in fade-in duration-200">
        <div className="bg-[var(--slack-bg)] rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] border border-[var(--slack-border)]">
          <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--slack-border)]">
            <h2 className="text-xl font-black text-slack-text">Themes</h2>
            <button
              onClick={onClose}
              className="p-2 text-[var(--slack-text-muted)] hover:bg-gray-100 dark:hover:bg-white/5 rounded-md transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="p-8 overflow-y-auto custom-scrollbar space-y-10">
            {/* Mode Selection */}
            <section>
              <h3 className="text-[15px] font-black text-slack-text mb-4 uppercase tracking-wider">
                Appearance
              </h3>
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  >
                    <Icon
                      className={`w-8 h-8 ${mode === id ? "text-[#1164A3]" : "text-[var(--slack-text-muted)]"}`}
                    />
                    <span
                      className={`text-[15px] font-bold ${mode === id ? "text-[#1164A3]" : "text-slack-text"}`}
                    >
                      {label}
                    </span>
                  </button>
                ...
                {Object.entries(SLACK_THEMES).map(([name, colors]) => (
                  ...
                  >
                    <div className="flex gap-2 h-16 w-full">
                      ...
                      ></div>
                      <div
                        className="w-12 rounded-lg shadow-inner"
                        style={{ backgroundColor: colors.active }}
                      ></div>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-[14px] font-bold capitalize text-slack-text">
                        {name}
                      </span>
                      {accentColor === colors.sidebar && (
