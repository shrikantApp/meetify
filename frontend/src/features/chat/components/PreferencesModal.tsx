...
export function PreferencesModal({
  ...
  const [activeSection, setActiveSection] =
    useState<PreferencesSectionId>(initialSection);
  ...
  const workspace = useAppSelector((state) =>
    state.workspace.workspaces.find((item) => item.id === activeWorkspaceId),
  );
  const members = useAppSelector((state) =>
    activeWorkspaceId
      ? (state.workspace.membersByWorkspace?.[activeWorkspaceId] ?? [])
      : [],
  );
  const invitations = useAppSelector((state) =>
    activeWorkspaceId
      ? (state.workspace.invitationsByWorkspace?.[activeWorkspaceId] ?? [])
      : [],
  );
  ...
  const sectionMeta = useMemo(
    () =>
      SECTION_ITEMS.find((item) => item.id === activeSection) ??
      SECTION_ITEMS[0],
    [activeSection],
  );
  ...
  const renderAppearance = () => (
    <div className="space-y-8">
      ...
      >
        <select
          className="preferences-select max-w-[240px]"
          defaultValue="lato"
        >
      ...
      <div className="border-b border-[var(--border-subtle)]">
        <div className="flex gap-6 text-sm font-bold">
          <button
            ...
            className={`pb-3 ${appearanceTab === "presets" ? "text-slack-text border-b-2 border-[var(--accent-primary)]" : "text-[var(--text-secondary)]"}`}
          >
            Slack themes
          </button>
          <button
            onClick={() => setAppearanceTab("custom")}
            className={`pb-3 ${appearanceTab === "custom" ? "text-slack-text border-b-2 border-[var(--accent-primary)]" : "text-[var(--text-secondary)]"}`}
          >
            Custom theme
          </button>
        </div>
      ...
      {appearanceTab === "presets" ? (
        ...
        <div className="space-y-5">
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={resetCustomTheme} className="preferences-chip">
              Reset
            </button>
          </div>
              ...
              <label key={key} className="preferences-color-card">
                ...
                <div className="mt-2 flex items-center gap-3 rounded-full border border-[var(--border-subtle)] bg-black/10 px-3 py-2">
                  <input
                    ...
                    onChange={(event) =>
                      updateCustomTheme({ [key]: event.target.value })
                    }
          ...
          <label className="flex items-start gap-3 rounded-xl border border-[var(--border-subtle)] bg-white/5 px-4 py-3">
            <input
              ...
              onChange={(event) =>
                updateCustomTheme({ windowGradient: event.target.checked })
              }
              className="mt-1"
            />
            <span>
              ...
              <span className="block text-xs text-[var(--text-secondary)]">
                Blend window background and selected item colors together for
                richer workspace surfaces.
              </span>
            </span>
          </label>
        </div>
      ...
      >
        {workspace ? (
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <PreferenceStat
              label="Members"
              value={String(workspace.memberCount ?? members.length)}
            />
            <PreferenceStat
              label="Your role"
              value={
                workspace.currentUserRole ?? currentMember?.role ?? "member"
              }
            />
          </div>
        ) : (
          ...
          <div key={member.id} className="preferences-list-item">
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold text-slack-text">
        ...
        >
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_160px_auto]">
            ...
            <button
              ...
              className="preferences-primary-button justify-center"
            >
              Invite
            </button>
          </div>
            ...
            <div key={invite.id} className="preferences-list-item">
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold">
                  {invite.inviteeEmail || invite.inviteeUserId}
                </p>
                <p className="truncate text-xs text-[var(--text-secondary)]">
    ...
    <div className="space-y-8">
      ...
      >
        <div className="space-y-3">
          {/* <button
            onClick={() =>
              activeWorkspaceId && dispatch(leaveWorkspace(activeWorkspaceId))
            }
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-rose-200/80">
                    Remove this workspace permanently. Conversations and
                    membership references may be lost.
                  </p>
                </div>
                <button
          ...
          >
            ...
            <section className="flex min-w-0 flex-1 flex-col bg-[var(--bg-base)]">
              <header className="flex items-center justify-between border-b border-[var(--border-subtle)] px-6 py-5">
                <div>
                  <h3 className="text-lg font-black text-slack-text">
                    {sectionMeta.label}
                  </h3>
                  <p className="mt-1 text-sm text-[var(--text-secondary)]">
                    {activeSection === "appearance"
                      ? "Control the look and feel of Meetify."
                      : "Manage this workspace with live data and role-aware actions."}
                  </p>
                </div>
...
function PreferenceGroup({
  ...
  return (
    <section className="space-y-4 border-b border-[var(--border-subtle)] pb-8">
      <div>
        <h4 className="text-sm font-black text-sidebar-text">{title}</h4>
...
function PreferenceStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-[var(--border-subtle)] bg-white/5 px-4 py-4">
      ...
      <p className="mt-2 text-base font-bold capitalize text-sidebar-text">
        {value}
      </p>
    </div>
  );
