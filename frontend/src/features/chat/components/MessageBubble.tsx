...
export function MessageBubble({
  ...
  const handleEdit = (newContent: string) => {
    ...
    dispatch(
      editMessage({ messageId: message.id, content: newContent.trim() }),
    );
    setIsEditing(false);
  };
        ...
        {isMeeting ? (
          ...
          (message.sender as any)?.avatar ? (
            <img
              src={
                ((message.sender as any)?.avatarUrl ||
                  (message.sender as any)?.avatar) as string
              }
...
function MessageContent({ content }: { content: string }) {
  ...
  const meetingMatch = content?.match(
    /Join my video call:\s*(https?:\/\/[^\s<]+)/i,
  );
        ...
        <div className="flex items-center gap-3 py-1">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5">
              <div
                className={`w-1.5 h-1.5 rounded-full ${isEnded ? "bg-[var(--text-muted)]" : "bg-emerald-500 animate-pulse"}`}
              />
...
function ActionButton({
  ...
  return (
    <Tooltip content={tooltip}>
      <button onClick={onClick} className="chat-icon-button p-1.5 rounded-lg">
        {icon}
      </button>
    </Tooltip>
  );
    ...
    return (
      ...
      >
        <img
          ...
          className="max-h-[300px] w-full object-cover bg-white/5"
        />
      </motion.div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-3 p-4 bg-[var(--surface-soft)] border border-[var(--border-subtle)] rounded-2xl max-w-sm group/att cursor-pointer hover:bg-[var(--surface-soft-hover)] transition-all glass-morphism">
      ...
      <a
        ...
        className="p-2 hover:bg-[var(--surface-soft-hover)] rounded-full transition-colors opacity-0 group-hover/att:opacity-100 text-[var(--text-muted)] hover:text-[var(--text-primary)]"
      >
