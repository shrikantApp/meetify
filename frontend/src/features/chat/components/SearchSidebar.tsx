...
export function SearchSidebar({ onClose }: Props) {
  ...
  const activeConv = useAppSelector(
    (state) => state.chat.conversations[state.chat.activeConversationId || ""],
  );
  ...
  return (
    <div className="w-[400px] flex-shrink-0 border-l border-[var(--slack-border)] bg-[var(--slack-bg)] flex flex-col overflow-hidden animate-in slide-in-from-right duration-200">
      <div className="h-[49px] px-4 flex items-center justify-between border-b border-[var(--slack-border)]">
        ...
        <button
          onClick={onClose}
          className="p-1 hover:bg-gray-100 dark:hover:bg-white/10 rounded text-[var(--slack-text-muted)]"
        >
          &times;
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
        {searchResults.length > 0
          ? searchResults.map((msg) => (
              <div
                key={msg.id}
                className="group p-3 rounded-lg border border-transparent hover:border-[var(--slack-border)] hover:bg-white dark:hover:bg-white/5 cursor-pointer transition-all"
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  (msg.sender as any)?.avatar ? (
                    <img
                      src={
                        ((msg.sender as any)?.avatarUrl ||
                          (msg.sender as any)?.avatar) as string
                      }
                ...
                <p className="text-[14px] leading-snug text-slack-text line-clamp-3">
                  {msg.content}
                </p>
              </div>
            ))
          : query && (
              <div className="text-center py-20">
                ...
                <p className="text-[13px] text-[var(--slack-text-muted)] font-medium">
