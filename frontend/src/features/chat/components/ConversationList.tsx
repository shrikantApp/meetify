...
export function ConversationList() {
  ...
  const { conversationOrder, activeConversationId } = useAppSelector(
    (state) => state.chat,
  );
  ...
  const groups = conversationOrder
    ...
    .filter(
      (c) => c && c.type === "group" && c.workspaceId === activeWorkspaceId,
    );

  const direct = conversationOrder
    .map((id) => conversations[id])
    .filter(
      (c) => c && c.type === "direct" && c.workspaceId === activeWorkspaceId,
    );
  ...
  return (
    <div className="w-[260px] flex-shrink-0 h-full bg-[var(--bg-sidebar)] border-r border-white/10 flex flex-col z-30">
      ...
      <div className="h-[52px] flex items-center justify-between px-4 border-b border-white/10">
        <div className="relative flex-1 min-w-0">
          ...
          >
            {activeWorkspace?.avatarUrl ? (
              ...
              <div className="w-7 h-7 rounded-lg premium-gradient flex items-center justify-center text-[var(--slack-sidebar-text)] text-[11px] font-black">
                {activeWorkspace?.name?.charAt(0).toUpperCase() || "W"}
              </div>
            )}
            <div className="min-w-0">
              <h1 className="text-[13px] font-bold truncate text-[var(--slack-sidebar-text)]">
                {activeWorkspace?.name || "Select Workspace"}
              </h1>
              <p className="text-[10px] text-sidebar-text 40 truncate">
                {activeWorkspace?.slug
                  ? `${activeWorkspace.slug}.slack.com`
              ...
              >
                <div className="flex items-center gap-3 border-b border-[var(--border-subtle)] px-4 py-4">
                  {activeWorkspace.avatarUrl ? (
                    ...
                    <div className="w-12 h-12 rounded-xl premium-gradient flex items-center justify-center text-[var(--slack-sidebar-text)] text-lg font-black">
        ...
        <button className="p-1.5 hover:bg-white/10 rounded-lg transition-colors text-sidebar-text 70">
          ...
          <div className="space-y-0.5">
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                >
                  <ChevronDown className="w-3 h-3 text-sidebar-text 40" />
                </motion.div>
                <span className="text-[10px] font-bold text-sidebar-text 40 uppercase tracking-widest">
                  Channels
                </span>
              </div>
              <button
                ...
                className="opacity-1 group-hover:opacity-100 p-0.5 hover:bg-black/5 dark:hover:bg-white/10 rounded transition-all text-sidebar-text 60"
              >
              ...
              {isChannelsOpen && (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-2 py-1 hover:bg-white/5 rounded-lg transition-all text-[11px] font-semibold text-sidebar-text 60 hover:text-[var(--slack-sidebar-text)] group"
                  >
                    <div className="w-4 h-4 rounded-md bg-white/5 flex items-center justify-center group-hover:bg-[var(--accent-primary)] group-hover:text-[var(--slack-sidebar-text)] transition-all">
          ...
          <div className="space-y-0.5">
            ...
            >
              ...
              <button
                ...
                className="opacity-1 group-hover:opacity-100 p-0.5 hover:bg-white/10 rounded transition-all text-sidebar-text 60"
              >
                <Plus className="w-3 h-3" />
              </button>
            </div>
      ...
      <WorkspaceEditorModal
        isOpen={isWorkspaceEditorOpen}
        onClose={() => setIsWorkspaceEditorOpen(false)}
      />
    </div>
  );
}
...
function SidebarLink({
  ...
  return (
    <div
      className={`flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg cursor-pointer transition-all duration-200 ${
        active
          ? "bg-[var(--accent-primary)] text-[var(--slack-sidebar-text)] shadow-lg shadow-[var(--accent-primary)]/20"
          : "hover:bg-white/5 text-sidebar-text 70 hover:text-[var(--slack-sidebar-text)]"
      }`}
    >
      <Icon
