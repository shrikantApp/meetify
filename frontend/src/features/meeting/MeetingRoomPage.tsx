...
export default function MeetingRoomPage() {
  ...
  const [lobbyGate, setLobbyGate] = useState<"checking" | "lobby" | "direct">(
    "checking",
  );
  ...
  const {
    ...
    isCamOn,
    isScreenSharing,
    layoutMode: (isSpotlightMode ? "spotlight" : "grid") as
      | "spotlight"
      | "grid",
  ...
  const pendingMediaState = useRef<{ camera: boolean; mic: boolean } | null>(
    null,
  );

  const handleRequestJoin = (mediaState: {
    camera: boolean;
    mic: boolean;
    screen: boolean;
  }) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-bg-primary text-text-primary overflow-hidden font-inter relative">
      ...
      <main className="flex flex-1 relative overflow-hidden px-6 pb-28">
        <div
          className={`flex-1 flex flex-col relative transition-all duration-500 ease-in-out ${showSidebar ? "mr-[380px]" : ""}`}
        >
            ...
            {!isSpotlightMode ? (
              ...
              <div className="flex-1 flex flex-wrap items-center justify-center gap-6 overflow-hidden content-center">
                {tiles.map((tile) => (
                  <div
                    key={tile.id}
                    className={`transition-all duration-700 ease-in-out flex items-center justify-center shrink min-h-0 ${
                      tiles.length === 1
                        ? "w-full h-full max-w-3xl"
                        : tiles.length === 2
                          ? "w-[calc(50%-12px)] h-full max-h-[85%]"
                          : tiles.length <= 4
                            ? "w-[calc(50%-12px)] h-[calc(50%-12px)]"
                    ...
                    <div className="w-full h-full max-h-full aspect-video flex items-center justify-center overflow-hidden rounded-2xl">
                      <VideoTile
                        ...
                        isMicOn={
                          tile.socketId === "local"
                            ? isMicOn
                            : (peerMediaStates[tile.socketId]?.mic ?? false)
                        }
                        isCamOn={
                          tile.socketId === "local"
                            ? tile.type === "camera"
                              ...
                              : true
                            : tile.type === "camera"
                              ? (peerMediaStates[tile.socketId]?.camera ??
                                false)
                              : true
                        }
                        ...
                        onContextMenu={() =>
                          openParticipantModal({
                            ...
                            isMicOn:
                              tile.socketId === "local"
                                ? isMicOn
                                : (peerMediaStates[tile.socketId]?.mic ??
                                  false),
                            isCamOn:
                              tile.socketId === "local"
                                ? isCamOn
                        ...
                        onClick={() =>
                          ...
                          openParticipantModal({
                            ...
                            isMicOn:
                              peerMediaStates[tile.socketId]?.mic ?? false,
                            isCamOn:
                              peerMediaStates[tile.socketId]?.camera ?? false,
                          })
                        }
                      />
                    </div>
                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          ...
                          {/* PiP Overlay */}
                          {pipTile &&
                            (pipTile.socketId === "local"
                              ? isCamOn && tile.stream
                              : peerMediaStates[pipTile.socketId]?.camera) && (
                              <div className="absolute bottom-6 right-6 w-48 aspect-video rounded-2xl overflow-hidden border-2 border-accent/40 shadow-2xl z-40 transition-all hover:scale-105 group-hover:translate-x-0 group-hover:translate-y-0">
                                <VideoTile
                                  ...
                                  isMicOn={
                                    pipTile.socketId === "local"
                                      ? isMicOn
                                      : (peerMediaStates[pipTile.socketId]
                                          ?.mic ?? false)
                                  }
                                  isCamOn={true}
                                  isMirrored={
                                    pipTile.socketId === "local" && isMirrored
                                  }
        ...
        <div
          className={`absolute top-0 right-0 h-full w-[380px] glass-panel !rounded-l-[2.5rem] !rounded-r-none z-40 transition-all duration-700 cubic-bezier(0.16, 1, 0.3, 1) shadow-2xl ${showSidebar ? "translate-x-0 opacity-100" : "translate-x-[420px] opacity-0"}`}
        >
              ...
              {sidebarTab === "participants" && (
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-accent/40 flex items-center justify-center font-bold">
                  ...
                  <div className="my-4 px-2 text-[10px] font-bold text-white/20 uppercase tracking-[0.2em]">
                    Other Participants ({peers.length})
                  </div>
                  {peers.map((p) => (
                    <div
                      key={p.socketId}
                      className="flex items-center justify-between p-4 glass-panel !bg-white/[0.02] hover:!bg-white/[0.08] rounded-2xl transition-all border-white/5 active:scale-[0.98] group cursor-pointer"
                    >
                      <div className="flex items-center gap-4">
