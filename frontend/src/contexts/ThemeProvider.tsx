...
export type ThemePresetId = 'aubergine' | 'huddle' | 'monument' | 'dark' | 'ocean';
...
export const SLACK_THEMES: Record<ThemePresetId, { label: string; primary: string; sidebar: string; active: string }> = {
  aubergine: { label: 'Aubergine', primary: '#3F0E40', sidebar: '#3F0E40', active: '#1164A3' },
  huddle: { label: 'Huddle', primary: '#1D1C1D', sidebar: '#1D1C1D', active: '#1164A3' },
  monument: { label: 'Monument', primary: '#004A5E', sidebar: '#004A5E', active: '#E8912D' },
  dark: { label: 'Gray', primary: '#1A1D21', sidebar: '#19171D', active: '#1164A3' },
  ocean: { label: 'Lagoon', primary: '#2D545E', sidebar: '#2D545E', active: '#C89666' },
};
...
const isThemePresetId = (value: string | null): value is ThemePresetId =>
  value === 'aubergine' || value === 'huddle' || value === 'monument' || value === 'dark' || value === 'ocean';
...
const parseCustomTheme = (): CustomThemeColors => {
  try {
    ...
    return {
      ...
      windowGradient: parsed.windowGradient ?? DEFAULT_CUSTOM_THEME.windowGradient,
    };
  } catch {
...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [customTheme, setCustomTheme] = useState<CustomThemeColors>(() => parseCustomTheme());
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--meetify-window-gradient', customTheme.windowGradient ? '1' : '0');
    ...
    if (activeTheme === 'dark') {
      ...
      root.style.setProperty('--slack-hover', '#F8F8F8');
    }
    ...
    <ThemeContext.Provider
      value={{
        ...
        setAccentColor: (color) => {
          const matchedPreset = (Object.entries(SLACK_THEMES).find(([, theme]) => theme.sidebar === color)?.[0] ?? DEFAULT_PRESET_ID) as ThemePresetId;
