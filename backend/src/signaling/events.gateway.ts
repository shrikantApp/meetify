...
@WebSocketGateway({
  cors: {
    origin:
      process.env.ALLOW_WEBSITE_URLS?.split?.(',') || 'http://localhost:5173',
    credentials: true,
  },
  namespace: '/signaling',
})
    ...
    OnModuleDestroy
{
  ...
  private removePendingRequest(client: Socket) {
    for (const [roomId, requestsMap] of this.pendingRequests.entries()) {
      if (requestsMap.has(client.id)) {
        ...
        this.logDebug('pending request removed', {
          socketId: client.id,
          roomId,
        });
        break;
  ...
  private isValidTarget(
    ...
    const target = this.getParticipant(
      { id: targetSocketId } as Socket,
      eventName,
    );
  ...
  private isHostOrCoHost(socketId: string, roomId: string): boolean {
    ...
    return (
      settings.hostSocketId === socketId ||
      settings.coHostSocketIds.has(socketId)
    );
  }
  ...
  private async admitParticipant(
    ...
    try {
      ...
      if (meeting && user) {
        const participant = await this.meetingsService.recordJoin(
          meeting,
          user,
        );
        const targetSocket = (this.server as any).sockets.get(targetSocketId);
        if (targetSocket) {
          targetSocket.dbParticipantId = participant.id;
        }
      }
    } catch (e: any) {
    ...
    for (const [roomId, requestsMap] of this.pendingRequests.entries()) {
      for (const [socketId, req] of requestsMap.entries()) {
        if (now - req.requestedAt > req.ttl) {
          ...
          if (settings?.hostSocketId) {
            this.server
              .to(settings.hostSocketId)
              .emit('join-request-cancelled', {
                socketId,
              });
          }
  ...
  async handleJoinRoom(
    ...
    for (const [rid, room] of this.rooms.entries()) {
      for (const [sid, info] of Object.entries(room.participants)) {
        if (info.userId === client.user.sub) {
          this.logDebug('removing stale session for user', {
            userId: info.userId,
            oldSocketId: sid,
          ...
          if (oldSocket) {
            ...
            this.server.to(rid).emit('user-left', {
              socketId: sid,
              userId: info.userId,
              userName: info.userName,
            });
    ...
    const isCoHost = settings
      ? [...settings.coHostSocketIds].some(
          (sid) =>
            this.rooms.get(data.roomId)?.participants[sid]?.userId ===
            client.user!.sub,
        )
      : false;
    ...
    this.logDebug('lobby-waiting sent', {
      socketId: client.id,
      roomId: data.roomId,
    });
  }
  ...
  handleJoinRequest(
    ...
    this.server
      .to(settings.hostSocketId)
      .emit('join-request-received', notification);
    for (const coHostId of settings.coHostSocketIds) {
  ...
  handleJoinRequestCancel(
    ...
    this.logDebug('join-request cancelled', {
      socketId: client.id,
      roomId: data.roomId,
    });
  }
    ...
    switch (data.action) {
      case 'mute-all': {
        ...
        for (const [sid, pInfo] of Object.entries(room.participants)) {
          if (
            sid !== settings.hostSocketId &&
            !settings.coHostSocketIds.has(sid)
          ) {
      ...
      case 'mute-participant': {
        ...
        if (targetSocket) {
          ...
          this.logDebug('force-mute sent to participant', {
            target: data.targetSocketId,
          });
        }
        break;
      ...
      case 'disable-camera': {
        ...
        if (targetSocket) {
          ...
          this.logDebug('force-disable-cam sent to participant', {
            target: data.targetSocketId,
          });
        }
        break;
      ...
      case 'end-meeting': {
        try {
          ...
          this.logDebug('meeting ended by host', {
            roomId: data.roomId,
            hostId: client.user!.sub,
          });
        } catch (e: any) {
  ...
  handleRenegotiateRequest(
    ...
    if (
      !this.isValidTarget(client.id, data.targetSocketId, 'renegotiate-request')
    )
      return;
